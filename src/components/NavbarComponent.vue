...
<template>
  <div v-if="!mobile" class="tw-flex tw-items-center tw-gap-3 font-open-sans">
    <div v-for="(nav, i) in NAVBAR_OPTIONS" :key="i" class="tw-col-span-2">
      <v-btn block size="large" :variant="navbarKey == nav.key ? 'flat' : 'text'" :color="formatColor(nav.key)"
        :class="formatClass(nav.key)" :append-icon="nav.key == 'submission' ? 'mdi-chevron-down' : ''"
        @click="onClickNav(nav.key)">
        <div class="tw-font-bold" :class="`${navbarKey == nav.key ? 'text-secondary-blue-dark' : ''}`">
          {{ nav.title }}
        </div>
      </v-btn>
    </div>
  ...
  <div v-else class="tw-w-full tw-text-center tw-mr-4">
    <v-menu width="200" :offset="[0, 30]" origin="overlap">
      <template #activator="{ props }">
        <v-btn color="secondary" size="large" variant="text" append-icon="mdi-chevron-down" v-bind="props">
      ...
      <v-list class="tw-shadow-lg tw-text-center">
        <v-list-item v-for="(item, index) in NAVBAR_OPTIONS" :key="index" :value="index"
          @click="onSelectSubnav(item.key)">
          <v-list-item-title :class="subNav == item.key ? 'text-primary-500 tw-font-semibold' : ''">{{ item.title }}
            <v-icon v-if="item.key == 'submission'" icon="mdi-chevron-down"></v-icon></v-list-item-title>
        </v-list-item>
      </v-list>
    </v-menu>
  </div>
